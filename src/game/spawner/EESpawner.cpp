...
EESpawner::EESpawner(Rectangle spawner_Area, Object_Manager& om)
  : Enemy_Spawner(spawner_Area, {}, nullptr,
                  game::EnemyConfig::kEESpawner_SpawnRate,
                  game::EnemyConfig::kEESpawner_MaxEnemies,
                  om),
    object_manager_ref(om),
    spawn_timer(game::EnemyConfig::kEESpawner_SpawnRate),
    max_enemies_per_instance_(game::EnemyConfig::kEESpawner_MaxEnemies)
{
...
void EESpawner::Update_And_Count_Spawned_Enemies(std::map<enemy::EnemyType, int>& counts)
{
    counts.clear();
    spawned_enemies_.erase(std::remove_if(spawned_enemies_.begin(), spawned_enemies_.end(),
        [&counts](const std::weak_ptr<enemy::Enemy_Base_Class>& e) {
            if (auto locked_enemy = e.lock()) {
                ...
                else if (std::dynamic_pointer_cast<enemy::DarknessMonster>(locked_enemy)) counts[enemy::EnemyType::PINGU]++;
                return false;
            }
            return true;
...
void EESpawner::Tick(float delta_time)
{
    ...
    if (spawn_timer <= 0.0f)
    {
        ...
        {
            const auto& enemy_pool = game::EnemyConfig::kEESpawner_EnemyPool;
...
void EESpawner::On_Collision(std::shared_ptr<Collidable> other)
{
    // Dieser Spawner ist unzerstörbar und reagiert nicht auf Kollisionen.
}

enemy::Enemy_Base_Class* EESpawner::create_enemy_by_type(Vector2 position, enemy::EnemyType type)
{
    switch (type) {
        ...
        case enemy::EnemyType::PINGU:               return new enemy::Pingu(position, object_manager_ref, true);
        default:                                    return nullptr;
    }
}

enemy::Enemy_Base_Class* EESpawner::createEnemy(Vector2 position)
{
